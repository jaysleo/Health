		...
		<view v-if="calendarData">
			...
			<view class="calendarBox">
				...
				<view class="actionBox u-flex u-row-center u-m-t-20">
					<u-button type="warning" shape="circle" plain size="mini" class="u-m-r-10" @click="addYaoLog">记录用药</u-button>
					<u-button type="warning" shape="circle" plain size="mini" class="u-m-r-10" @click="addPisunqingchuLog">记录皮损清除</u-button>
					<u-button type="warning" shape="circle" plain size="mini" class="u-m-r-10" @click="addPisunfufaLog">记录皮损复发</u-button>
				</view>
			</view>
		</view>
...
export default {
	...
	methods: {
		addYaoLog() {
			let url = 'https://ifufan.com/xs_app/detail/0e12/Medical?random=' + Math.random() + '&uid=' + uni.getStorageSync('phone') + '&currentDate=' + this.today();
			uni.navigateTo({
		...
		editYao(recordId) {
			let url = 'https://ifufan.com/xs_app/detail/0e12/Medical?random=' + Math.random() + '&phone=' + uni.getStorageSync('phone') + '&id=' + recordId ;
			// let url =
		...
		delYao(recordId) {
			wx.request({
				url: app.globalData.doctorHttps + '/deleteFormDetail',
				method: 'POST',
				data: {
					templateid:'0e12',
					mid:'Medical',
		...
		addPisunqingchuLog() {
			let url = 'https://ifufan.com/xs_app/detail/0e12/SkinLesion?random=' + Math.random() + '&type=1&uid=' + uni.getStorageSync('phone');
			uni.navigateTo({
				url: '/pages/h5/h5?url=' + encodeURIComponent(JSON.stringify(url))
			});
		},
		...
		addPisunfufaLog() {
			let url = 'https://ifufan.com/xs_app/detail/0e12/SkinLesion_n?random=' + Math.random() + '&type=2&uid=' + uni.getStorageSync('phone');
			uni.navigateTo({
				url: '/pages/h5/h5?url=' + encodeURIComponent(JSON.stringify(url))
			});
		},
		...
		dayChange(e) {
			if (e.info) {
				...
				let url = 'https://ifufan.com/xs_app/detail/0e12/Medical?uid=' + uni.getStorageSync('phone') + '&date=' + e.fullDate;
				uni.navigateTo({
					url: '/pages/h5/h5?url=' + encodeURIComponent(JSON.stringify(url))
				});
			}
