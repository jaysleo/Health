<template>
	<view>
		<view v-if="calendarData">
			<view class="planBox" @click="editYao">
				<view class="u-flex u-row-between">
					<view>
						<u-icon name="checkmark-circle" class="u-m-r-10"></u-icon>
						用药方案
						<u-tag text="服药记录" shape="circle" class="u-m-l-10" border-color="#36cfc9" color="#36cfc9" mode="plain" />
					</view>
					<!-- <view>
...
export default {
	...
	methods: {
				...
				success: res => {
					...
					if (res.data.success) {
						...
						for (let i in res.data.result) {
							let name = '';
							for (let j in res.data.result[i].yaopin) {
		...
		addYaoLog() {
			let url = 'https://ifufan.com/xs_app/detail/ca3f/pinggu?random=' + Math.random() + '&uid=' + uni.getStorageSync('phone') + '&currentDate=' + this.today();
			uni.navigateTo({
		...
		editYao(recordId) {
			let url = 'https://ifufan.com/xs_app/detail/ca3f/pinggu-list?random=' + Math.random() + '&uid=' + uni.getStorageSync('phone') + '#/edite/' + recordId;
			// let url =
		...
		dayChange(e) {
			...
			// 	}
			// } else {
				let url = 'https://ifufan.com/xs_app/detail/ca3f/pinggu-list?uid=' + uni.getStorageSync('phone') + '&currentDate=' + e.fullDate + '#/add';
				uni.navigateTo({
					url: '/pages/h5/h5?url=' + encodeURIComponent(JSON.stringify(url))
				});
			// }
